...
describe("CryptoBeatERC721", async () => {
    ...
    describe("Mint", async () => {
        it("Should mint", async () => {
            ...
            const tokenId = await cryptoBeat.connect(randomGuy).callStatic.mint(tokenURISuffix);
            await cryptoBeat.connect(randomGuy).mint(tokenURISuffix);

            expect(await cryptoBeat.balanceOf(randomGuy.address)).eq(BigNumber.from(1));
            expect(await cryptoBeat.ownerOf(tokenId)).eq(randomGuy.address);
            expect(await cryptoBeat.tokenURI(tokenId)).eq(_baseTokenURI + tokenURISuffix);
        });
    });
});